/**
 * 前端日志：dev 走 console；prod 错误上报后端写进 app.log（与后端日志统一回溯），警告只进 Sentry。
 *
 * 上报走裸 fetch（不经 apiFetch，避免 401 拦截卡住登录页崩溃上报）；keepalive 让页面 unload 也能发出。
 * 节流防死循环刷爆带宽：同 message+source 10s 内只发一次，全局 30s 内最多 20 条（与后端限流对齐）。
 */
...
const RATE_MAX = 20; // 窗口内最多上报条数（与后端 client_error_rate_limit 对齐）
...
/** 记录错误：dev 走 console；prod 上报后端 app.log + Sentry（无 DSN 时 no-op）。 */
export function logError(error: unknown, context: ErrorContext = {}): void {
  ...
  const stack = err.stack + (context.componentStack ? `\n${context.componentStack}` : "");
