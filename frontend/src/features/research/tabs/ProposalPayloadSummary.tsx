...
export function ProposalPayloadSummary({ payload, topic }: ProposalPayloadSummaryProps) {
  ...
  if (!hasSummary) {
    return (
      <div className="mt-3 rounded-panel border border-border/70 bg-card/70 p-3 text-fine leading-relaxed text-muted-foreground">
  ...
  return (
    <div className="mt-3 space-y-3 rounded-panel border border-border/70 bg-card/70 p-4">
      ...
      {sourceIds.length > 0 && (
        <div>
          <div className="mb-1.5 text-fine font-bold text-foreground">新增来源</div>
          <div className="space-y-1.5">
            {sourceIds.map((id) => {
              const source = topic.sources.find((item) => item.id === id);
              return (
                <div key={id} className="rounded-xl bg-background/70 px-3 py-2 text-fine text-muted-foreground">
      ...
      {claimIds.length > 0 && (
        <div>
          <div className="mb-1.5 text-fine font-bold text-foreground">新增事实</div>
          <div className="space-y-1.5">
            {claimIds.map((id) => {
              const claim = topic.claims.find((item) => item.id === id);
              return (
                <div key={id} className="rounded-xl bg-background/70 px-3 py-2 text-fine leading-relaxed text-muted-foreground">
      ...
      {conflicts.length > 0 && (
        <div>
          <div className="mb-1.5 text-fine font-bold text-destructive">发现冲突</div>
          <div className="space-y-1.5">
            {conflicts.map((conflict, index) => (
              <div key={`${conflict.between}-${index}`} className="rounded-xl border border-destructive/15 bg-destructive/5 px-3 py-2 text-fine leading-relaxed text-muted-foreground">
          ...
          <p className="mt-2 text-fine leading-relaxed text-muted-foreground">建议先处理冲突，再把相关事实写成确定结论。</p>
        </div>
      )}
    </div>
  );
