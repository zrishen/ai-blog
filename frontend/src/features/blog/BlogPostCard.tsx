...
function MetaInfo({ post, className }: { post: BlogPost; className?: string }) {
  ...
  return (
    <div className={`flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] ${className || "text-muted-foreground"}`}>
      {post.author && (
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {tags.map((tag) => (
        <span
          key={tag}
          className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${onDarkImage ? "border-white/22 bg-white/14 text-white/82 backdrop-blur-md" : ""}`}
    ...
    return (
      ...
      >
        ...
        <div className={`absolute inset-x-0 bottom-0 p-6 sm:p-8 ${coverDark ? "text-white" : "text-foreground"}`}>
          ...
          {excerpt && (
            <p className={`mt-3 text-sm leading-relaxed line-clamp-2 ${coverDark ? "text-white/72" : "text-muted-foreground"}`}>
    ...
    >
      <div className="flex min-w-0 flex-1 flex-col p-4">
        <div className="flex flex-wrap items-center gap-1 overflow-hidden">
          {post.status === "draft" && (
            <Badge variant="outline" className="rounded-full border-amber-400/30 bg-amber-500/10 text-amber-600 text-[10px] px-1.5 py-0">
              草稿
            </Badge>
          )}
          {tags.length > 0 && <TagList tags={tags} />}
