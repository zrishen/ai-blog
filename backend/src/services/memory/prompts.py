...
偏好（preferences）只抽用户表达的长期倾向、习惯、约束或好恶（如偏好的回复语言、写作或代码风格、技术栈、饮食/过敏、作息）；一次性请求或当前任务不算偏好。key 用简短名词短语表示偏好类别，value 为具体内容。"""

EXTRACTION_USER_LIGHT = """抽取实体（entities）和情景（episodes）。实体给出 name/type/description；不必抽事实。"""

EXTRACTION_USER_DEEP = """深度抽取：实体（entities）+ 事实（facts，主谓宾三元组）+ 情景（episodes）+ 偏好（preferences）。
事实要求：
