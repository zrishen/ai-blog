...
async def cleanup_expired_pending_attachments(
    ...
    *,
    ...
    expired = list(
        (
            await db.execute(
                select(ChatAttachment.id, ChatAttachment.stored_path)
                .where(
    ...
    for attachment_id, stored_path in expired:
        result = await db.execute(
            ...
            .returning(ChatAttachment.stored_path)
        )
        deleted_path = result.scalar_one_or_none()
        if deleted_path:
    ...
    for stored_path in deleted_paths:
        try:
            await _safe_unlink(_resolve_stored_path(stored_path))
...
async def create_or_reuse_attachment(
    ...
    *,
    ...
    user_dir = _resolve_stored_path(user_relative_dir.as_posix())
    ...
    final_path = _resolve_stored_path(stored_path)
...
async def prepare_claimed_attachments(
    ...
    for position, attachment in enumerate(attachments):
        ...
        path = _resolve_stored_path(attachment.stored_path)
