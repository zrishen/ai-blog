...
SYSTEM_SMART_THINKING = (
    "\n\n当前处于智能模式。回答前要先充分理解用户目标、上下文和约束；"
    "复杂任务要分步骤分析，给出依据、取舍、风险和结论；"
    "如果使用工具或文件库，必须结合实际结果再回答。"
...
# 注入工具名列表(运行时 .format(names=...))
SYSTEM_TOOL_RULES = (
    "\n\n可用工具：{names}。"
    "调用工具前，简要说明你的思路。"
    "只在需要时调用工具，完成后直接回复用户，最终回复可以按照情况选择性详细。"
    "连续调用同一工具失败时，不要重试超过 3 次。"
    "处理博客时，局部修改使用 blog_edit_post，修改文章属性或重写全文使用 blog_write_post。"
    "title 只包含标题，content 只包含正文；正文不要重复一级标题，章节标题从 ## 开始。"
    "修改指定章节时，先读取大纲和目标章节，再进行精准替换。"
    "列出或搜索文章，以及定位重复文本时，使用 blog_search_posts。"
)
