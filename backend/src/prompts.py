...
SYSTEM_TOOL_RULES = (
    ...
    "文章正文支持 ```mermaid 代码块，需要流程图、时序图、架构图、类图等示意图时可直接在正文中写 mermaid 代码（前端会渲染成图）；仅在确有帮助时使用，不要为加图而加图。"
    "写 mermaid 时注意美观与可读：按内容选合适方向（步骤多且线性用 LR，层级归属用 TD）；"
    "用语义化节点形状（决策用菱形{}、起止用圆角()、数据存储用圆柱[( )]）；相关节点用 subgraph 分组；"
    "用 classDef 为不同角色或状态配区分色，避免全部默认色；边上标注条件（如 是/否）；单图节点不宜过多，必要时拆成多张图。"
)
