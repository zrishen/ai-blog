...
async def test_file_document_restore_requires_source_file(client: AsyncClient, db_session: AsyncSession, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.services.trash.trash_service.get_user_upload_dir",
        lambda uid: fake_dir,
    )
...
async def test_file_purge_unlink_failure_still_deletes_record(
    ...
    monkeypatch.setattr("src.services.trash.trash_service.get_user_upload_dir", lambda user_id: tmp_path)
...
async def test_empty_trash_partial_keeps_failed_item(client: AsyncClient, monkeypatch):
    ...
    monkeypatch.setattr("src.services.trash.trash_service._purge_conversation", _boom)
