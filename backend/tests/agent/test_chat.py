...
async def test_build_messages_uses_empty_history_for_invalid_conversation(monkeypatch):
    from src.services.agent import messages
    from src.services.agent import orchestrator as chat_service
...
async def test_reasoning_content_debug_log_is_aggregated(monkeypatch, caplog):
    ...
    caplog.set_level(logging.DEBUG, logger="src.services.agent.orchestrator")
...
def test_build_page_context_parts_gates_writing_instructions_by_tool_mount():
    ...
    from src.services.agent.orchestrator import _build_page_context_parts
...
def test_partial_int_waits_for_json_number_delimiter():
    from src.services.agent.streaming import _extract_partial_int
...
def test_build_current_user_content_formats_provider_images_and_documents():
    from src.services.agent import orchestrator as chat_service
    from src.services.agent.chat_attachment_service import PreparedChatAttachment
