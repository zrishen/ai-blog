...
@pytest.mark.asyncio
async def test_mismatched_stored_path_cannot_read_or_cleanup_another_users_file(
    client: AsyncClient,
    db_session: AsyncSession,
):
    ...
    other_user = User(username="attachment-path-other", password_hash="mock")
    db_session.add(other_user)
    await db_session.flush()

    protected_path = Path(settings.chat_attachment_dir) / str(other_user.id) / "protected.txt"
    ...
    assert await cleanup_expired_pending_attachments(db_session) == 1
    assert protected_path.read_text(encoding="utf-8") == "protected"
